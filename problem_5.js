...
function startAnimation(blockEl) { // blockEl is a DOM element.
    const duration = 1000; // The animation runs for 2 seconds
    ...
    function step() {
        ...
        const position = pctElapsed;                          // assign the position to be a number between 0 and 1
        ...
        } // If we haven't finished the animation, run another step
        else { 
            // probably need to do something here but I'm not really sure what. 
            //requestAnimationFrame(step);


            setTimeout(() => setX(0), 2000); 
        } // Reset the animation location after one second
    }
}
